...
const fetch = async (_a: any, _b: any, options: FetchOptions): Promise<FetchResultFees> => {
  ...
  const poolsForDay: { [poolName: string]: { totalFees: string } } = res[targetDate];
  ...
  let dailyFees = 0;
  for (const [poolName, stats] of Object.entries(poolsForDay)) {
    const poolFees = (Number(stats.totalFees) / 1e6) || 0;
    poolFeesData[poolName] = poolFees;
    dailyFees += poolFees;
  }
  ...
  if (timestamp > holderRevenueStartTimestamp) {
    ...
    for (const pool of Object.keys(poolRevenueRates)) {
...
const methodology = {
  Fees: 'All trading fees generated across all pools.',
